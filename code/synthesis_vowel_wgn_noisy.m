...
noise1 = wgn(length(reconst1),1,(10*log10((rms(reconst1)^2)/(10^(required_snr/10)))))';
snrr = snr(reconst1,noise1);
noisy_signal = reconst1 + noise1;



audiowrite('single_channel_stimuli.wav',4*noisy_signal,fs,'Bitspersample',16);
%cnt1 = cnt1+1;
% %
player = audioplayer(4*noisy_signal,fs);
play(player);
